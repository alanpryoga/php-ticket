...
class TicketController
{
    ...
    public function generateTickets(array $argv)
    {
        ...
        if ($eventId == null || is_int($eventId) == false || $eventId == 0) {
            printf("\033[31m{event_id} must be number and mandatory\033[0m\n");
            $hasErrors = true;
        }

        if ($n == null || is_int($n) == false || $n == 0) {
            printf("\033[31m{n} must be number and mandatory\033[0m\n");
            $hasErrors = true;
        }
        ...
        if (! $ok) {
            printf("\033[31mFailed to generate tickets\033[0m\n");
            return;
        }
